...
async def send_movie_update(bot, base_name):
    ...
    for attempt in range(max_retries):
        try:
            ...
            InlineKeyboardButton(
                'ɢᴇᴛ ғɪʟᴇs',
                url="https://t.me/The_hell_king_movie_group"
            )
...
def generate_movie_message(movie_doc, base_name):
    ...
    hashtag = f"#{base_name.upper().replace(' ', '_')}"
    ...
    if genres != "N/A":
        for g in genres.split(","):
            genre_clean = g.strip().replace("-", "") # Film-Noir -> FilmNoir
            if genre_clean:
                genre_hashtags.append(f"#{genre_clean}")
    genre_hashtags_text = ' '.join(genre_hashtags) if genre_hashtags else "N/A"




    latest_file = max(movie_doc["files"], key=lambda f: f.get("timestamp", datetime.min))
# Convert timestamp to Asia/Kolkata timezone
    kolkata_tz = pytz.timezone("Asia/Kolkata")
    kolkata_time = latest_file.get("timestamp").astimezone(kolkata_tz)
    timestamp = kolkata_time.strftime("%d %B %Y, %I:%M %p")
