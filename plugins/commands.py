...
async def start(client, message):
    ...
    if len(m.command) == 2 and m.command[1].startswith(('notcopy', 'sendall')):
        ...
        await client.send_message(settings['log'], script.VERIFIED_LOG_TEXT.format(m.from_user.mention, user_id, datetime.datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%d %B %Y'), num))
        btn = [[
...
# This block replaces the file sending logic in your 'start' function
    user_id = message.from_user.id
    # --- Logic for "Send All" Button ---
# This is the FINAL replacement block for the end of your 'start' function
    ...
    if data.startswith("allfiles"):
        try:
            ...
            if not files:
                return await message.reply_text('<b><i>ɴᴏ ꜱᴜᴄʜ ꜰɪʟᴇ ᴇxɪꜱᴛꜱ !</b></i>', quote=True)

            user_id = message.from_user.id
            is_exempt = user_id in ADMINS or await db.has_premium_access(user_id)
            
            if not is_exempt:
                current_count = await db.get_daily_limit(user_id)
                if current_count >= DAILY_FILE_LIMIT:
                    await message.reply_text(f"🚫 **Daily Limit Already Reached** 🚫\n\nYou have already used your **{DAILY_FILE_LIMIT} files** for today.", quote=True)
                    return
                remaining_slots = DAILY_FILE_LIMIT - current_count
                files_to_send = files[:remaining_slots]
                limit_will_be_reached = len(files) > len(files_to_send)
            else:
                # Admins and premium users get all files
                files_to_send = files
                limit_will_be_reached = False

            sent_messages = []
            # Loop and send files quickly without waiting
            for file in files_to_send:
                sent_msg = await send_file_with_limit(client, message, file.file_id, grp_id, schedule_deletion=False)
                if sent_msg and sent_msg != "not_found":
                    sent_messages.append(sent_msg)
                elif sent_msg is None: # Limit reached
                    break
            
            # After sending, if any files were sent, schedule their deletion
            # if sent_messages:
            #     asyncio.create_task(delete_messages_after_delay(sent_messages, DELETE_TIME))
                
            #     final_text = f"✅ **{len(sent_messages)} files sent!** They will be deleted in {get_time(DELETE_TIME)} for security."
            #     if limit_will_be_reached:
            #         final_text += "\n\nYou have now reached your daily limit. Some files were not sent."
    ...
    # --- Logic for Single File Button ---
    # This now works perfectly by itself
    await send_file_with_limit(client, message, file_id, grp_id, schedule_deletion=True)
    return
...
async def set_tutorial(client, message: Message):
    ...
    try:
        ...
        return await message.reply_text(
            ...
            f"<code>/{message.command[0]} https://t.me/the_hell_king_updates</code>"
        )
...
# Add this new function in commands.py

@Client.on_message(filters.private & filters.command("sendall_mode") & filters.user(ADMINS))
async def set_sendall_mode(client, message):
    ...
    try:
        ...
        await message.reply_text("<b>💔 Invalid option. Please use 'on' or 'off' after the command.</b>\n\nExample: `/sendall_mode on`")
        return
    try:
        await db.update_sendall_mode_status(bot_id, enable_status)
        response_text = (
            "<b>✅ 'Send All' button is now enabled for <u>Premium Users Only</u>.</b>" if enable_status
            else "<b>❌ 'Send All' button is now enabled for <u>All Users</u>.</b>"
        )
        await message.reply_text(response_text)
    except Exception as e:
...
# Add this entire block of code at the end of commands.py

def parse_duration(duration_str: str):
    """Parses a duration string (e.g., '1 month', '30 days') into a timedelta object."""
    duration_str = duration_str.lower().strip()
    match = re.match(r'(\d+)\s*(day|hour|min|month|year)s?', duration_str)
...
def format_time_left(td: datetime.timedelta):
...
async def add_premium_command(client, message):
    usage_text = (
        "<b>📌 ᴜsᴀɢᴇ:</b> <code>/add_premium user_id time</code>\n"
        "<b>📅 ᴇxᴀᴍᴘʟᴇ:</b> <code>/add_premium 123456 1 month</code>\n"
        "<b>🧭 ᴀᴄᴄᴇᴘᴛᴇᴅ ꜰᴏʀᴍᴀᴛs:</b> 1 day, 1 hour, 1 min, 1 month, 1 year"
    )
    ...
    if not duration:
        return await message.reply_text(f"<b>❌ Invalid time format.</b>\n\n{usage_text}")

    expiry_time = datetime.datetime.now() + duration
    ...
    try:
        ...
        await message.reply_text(
            ...
            f"⌛ **Plan added:** {duration_str}\n"
            f"🗓️ **Expires on:** `{expiry_time.strftime('%d-%b-%Y %I:%M %p')}`"
        )
        ...
        try:
            await client.send_message(
                ...
                text=(
                    ...
                    "You have been upgraded to a **Premium User**!\n\n"
                    f"Your subscription is valid for **{duration_str}** and will expire on:\n"
                    f"`{expiry_time.strftime('%d-%b-%Y %I:%M %p')}`\n\n"
                    "Enjoy the exclusive benefits!"
                )
            )
        except Exception:
            await message.reply_text("ℹ️ Could not send notification to the user (they may have blocked the bot).")
        ...
        if PREMIUM_LOGS:
            await client.send_message(
                ...
                f"⌛ **Plan:** {duration_str}\n"
                f"🗓️ **Expires:** `{expiry_time.strftime('%d-%b-%Y')}`\n"
                f"👨‍💻 **Admin:** {message.from_user.mention}"
            )

    except Exception as e:
        await message.reply_text(f"<b>Error:</b> Failed to find user or update database.\n`{e}`")


@Client.on_message(filters.command("remove_premium") & filters.user(ADMINS))
async def remove_premium_command(client, message):
    usage_text = "<b>📌 ᴜsᴀɢᴇ:</b> <code>/remove_premium user_id</code>"
    ...
    try:
        user = await client.get_users(user_id)
        if not await db.has_premium_access(user_id):
            return await message.reply_text(f"❌ This user does not have an active premium subscription.")

        await db.remove_premium_access(user_id)
        
        # Notify Admin
        await message.reply_text(f"✅ **Success!** Premium access for {user.mention} (`{user.id}`) has been revoked.")

        # Notify User
        try:
            await client.send_message(
                chat_id=user_id,
                text="⚠️ **Subscription Revoked**\n\nYour premium access has been removed by an administrator."
            )
        except Exception:
...
async def premium_users_command(client, message):
    msg = await message.reply_text("⏳ Fetching premium users list...")
    
    users_cursor = await db.all_premium_users()
    users_list = await users_cursor.to_list(length=None) # Get all users
    
    if not users_list:
        return await msg.edit("<b>텅 비어있는! (Empty!)</b>\n\nThere are currently no active premium users.")

    output = "✨ **Active Premium Users** ✨\n\n"
    count = 0
    for user_doc in users_list:
        user_id = user_doc.get("id")
        expiry_time = user_doc.get("expiry_time")
        ...
        if time_left.total_seconds() > 0:
            ...
            output += (
                f"**{count}. {mention}**\n"
                f"   - **ID:** `{user_id}`\n"
                f"   - **Time Left:** `{format_time_left(time_left)}`\n\n"
            )
...
# Add this entire function at the end of commands.py

async def send_limited_file(client, user_id, file_id, grp_id):
    """
    This is the central function for sending files.
    It checks the daily limit, formats the caption, and sends the file.
    Returns False if the limit is reached, True otherwise.
    """
    # --- 1. Check User Status and Daily Limit ---
    is_exempt = user_id in ADMINS or await db.has_premium_access(user_id)
    current_count = await db.get_daily_limit(user_id)
    
    if not is_exempt and current_count >= DAILY_FILE_LIMIT:
        # User has reached the limit, send a notification
        await client.send_message(
            chat_id=user_id,
            text=(
                f"🚫 **Daily Limit Reached** 🚫\n\n"
                f"You have already received your quota of **{DAILY_FILE_LIMIT} files** for today.\n\n"
                "Upgrade to a **✨ Premium Plan** to get:\n"
                "✅ Unlimited daily files\n"
                "✅ Ad-free experience\n"
                "✅ Direct stream & download links\n\n"
                "Use the `/premium` command to learn more!"
            ),
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("➕ Upgrade to Premium ➕", callback_data="premium_info")
            ]])
        )
        return False # Signal that sending failed

    # --- 2. Increment Count for Non-Exempt Users ---
    if not is_exempt:
        await db.increment_daily_limit(user_id)
        limit_text = f"{current_count + 1}/{DAILY_FILE_LIMIT}"
    else:
        limit_text = "Unlimited 👑"

    # --- 3. Fetch File Details and Settings ---
    files_ = await get_file_details(file_id)
    if not files_:
        await client.send_message(user_id, 'ɴᴏ ꜱᴜᴄʜ ꜰɪʟᴇ ᴇxɪꜱᴛꜱ !')
        return True # Not a limit failure, just a missing file
    ...
    # --- 4. Generate the New Custom Caption ---
    f_caption = (
        f'<b><a href="https://t.me/Hell_king_69_Bot"><blockquote>{title}</blockquote></a></b>\n\n'
        f'<b>Powered By ⚜️ :<a href="https://t.me/Hell_king_69_Bot"> The Hell King</a>\n Size ⚙️: {size}</b>\n\n'
        f'<blockquote> Dᴀɪʟʏ Fɪʟᴇ Lɪᴍɪᴛ: {limit_text}</blockquote>\n\n'
        f'<b>⚠️ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <u>{get_time(DELETE_TIME)}</u> </b>\n\n'
        '<b><i>ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ᴛʜɪꜱ ꜰɪʟᴇ ᴛᴏ ꜱᴏᴍᴇᴡʜᴇʀᴇ ᴇʟꜱᴇ ᴀɴᴅ ꜱᴛᴀʀᴛ ᴅᴏwɴʟᴏᴀᴅɪɴɢ ᴛʜᴇʀᴇ</i></b>'
    )

    # --- 5. Prepare Buttons (Stream, etc.) ---
    btn = []
    # if STREAM_MODE:
    if await db.watch_mode_status(client.me.id):
        is_premium = await db.has_premium_access(user_id)
        if not PREMIUM_STREAM_MODE or is_premium:
    ...
    btn.append([InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=UPDATE_CHNL_NK)])

    # --- 6. Send the File and Schedule Deletion ---
    ...
    # The auto-deletion message is now optional to reduce spam, you can add it back if you want
    # k = await msg.reply(...)
    await asyncio.sleep(DELETE_TIME)
    try:
        await msg.delete()
    ...
    # await k.edit_text(...)
    return True # Signal that sending was successful





# REPLACE your existing send_file_with_limit function with this FINAL version

# REPLACE your existing send_file_with_limit function with this FINAL, SIMPLIFIED version
    ...
    #     f'<b>Powered By ⚜️ :<a href="https://t.me/Hell_king_69_Bot"> The Hell King</a>\n Size ⚙️: {size}</b>\n\n'
    #     f'<blockquote> Dᴀɪʟʏ Fɪʟᴇ Lɪᴍɪᴛ: {limit_text}</blockquote>\n'
    #     f'⚠️ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ {get_time(DELETE_TIME)}\n\n'
    #     f' ᴘʟᴇᴀꜱᴇ ꜰᴏʀᴡᴀʀᴅ ᴛʜɪꜱ ꜰɪʟᴇ ᴛᴏ ꜱᴏᴍᴇᴡʜᴇʀᴇ ᴇʟꜱᴇ ᴀɴᴅ ꜱᴛᴀʀᴛ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴛʜᴇʀᴇ ')
    

# REPLACE your existing send_file_with_limit function with this FINAL, CORRECTED version

async def send_file_with_limit(client, message, file_id, grp_id, schedule_deletion=True):
    """
    Central function for sending files.
    - Fixes all caption issues (links, content, typo).
    - Uses the new, simple /stream_mode setting.
    """
    user_id = message.from_user.id
    bot_id = client.me.id
    ...
    current_count = await db.get_daily_limit(user_id)
    if not is_exempt and current_count >= DAILY_FILE_LIMIT:
        await message.reply_text(f"🚫 **Daily Limit Reached** 🚫\nYou have already used your **{DAILY_FILE_LIMIT} files** for today.", quote=True)
        return None
    ...
    files_ = await get_file_details(file_id)
    if not files_: return "not_found"
    file_info = files_[0]
    title = clean_filename(file_info.file_name)
    size = get_size(file_info.file_size)
    settings = await get_settings(int(grp_id))

    # --- 4. Generate CORRECTED Caption ---
    f_caption = (
        f'<b><a href="https://t.me/Hell_king_69_Bot">{title}</a></b>\n\n'
        f'<b>Powered By ⚜️ : <a href="https://t.me/Hell_king_69_Bot">The Hell King</a>\nSize ⚙️: {size}</b>\n\n'
        f'<blockquote>Dᴀɪʟʏ Fɪʟᴇ Lɪᴍɪᴛ: {limit_text}</blockquote>\n'
        f'<b>⚠️ ꜰɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇ ᴀꜰᴛᴇʀ <u>{get_time(DELETE_TIME)}</u></b>\n\n'
    ...
    # --- 5. Prepare Buttons with NEW Logic ---
    btn = []
    stream_mode = await db.get_stream_mode(bot_id)
    
    show_button = False
    if stream_mode == "all":
        show_button = True
    elif stream_mode == "premium":
        show_button = user_id in ADMINS or await db.has_premium_access(user_id)
    # If mode is "off", show_button remains False
    ...
    msg = await client.send_cached_media(
        ...
        protect_content=settings.get('file_secure', PROTECT_CONTENT),
        reply_markup=InlineKeyboardMarkup(btn),
        parse_mode=enums.ParseMode.HTML # Explicitly set parse mode to fix links
    )
    
    # --- 7. Handle Deletion ---
    if schedule_deletion:
        asyncio.create_task(delete_messages_after_delay([msg], DELETE_TIME))
        return msg
    else:
        return msg
...
# Add this new function at the end of commands.py

async def delete_messages_after_delay(messages, delay):
    """A helper coroutine to delete a list of messages after a specified delay."""
...
# Add this new function in commands.py, replacing the old /watch and /watch_mode

@Client.on_message(filters.private & filters.command("stream_mode") & filters.user(ADMINS))
async def set_stream_mode(client, message):
    bot_id = client.me.id
    try:
        option = message.text.split(" ", 1)[1].strip().lower()
    except (IndexError, ValueError):
        await message.reply_text("<b>💔 Invalid command.</b>\n\n<b>Usage:</b> `/stream_mode [all | premium | off]`")
        return

    if option not in ["all", "premium", "off"]:
        await message.reply_text("<b>❌ Invalid mode.</b> Please choose from `all`, `premium`, or `off`.")
        return

    await db.update_stream_mode(bot_id, option)
    
    response_text = {
        "all": "✅ **Stream mode set to <u>ALL</u>.**\nEveryone will now get the watch/download button.",
        "premium": "✨ **Stream mode set to <u>PREMIUM</u>.**\nOnly Admins and Premium Users will get the watch/download button.",
        "off": "❌ **Stream mode is now <u>OFF</u>.**\nNo one will get the watch/download button."
    }[option]
