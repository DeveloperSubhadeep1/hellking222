...
class Database:    
    def __init__(self, uri, database_name):
        ...
        # Add this line inside the __init__ method
        self.daily = self.db.daily_limits
...
# # Add this new function inside the Database class in users_chats_db.py

    async def all_premium_users(self):
        # Find users whose expiry_time is greater than the current time
        return self.users.find({"expiry_time": {"$gt": datetime.datetime.now()}})
        


# Add these two new functions inside the Database class in users_chats_db.py

    async def get_premium_users_count(self):
        """Returns the total number (integer) of active premium users."""
        count = await self.users.count_documents({
            "expiry_time": {"$gt": datetime.datetime.now()}
        })
        return count

    async def get_all_premium_users_cursor(self):
        """Returns a cursor to iterate over all active premium users."""
        return self.users.find({"expiry_time": {"$gt": datetime.datetime.now()}})
...
#     async def watch_mode_status(self, bot_id):
#         # Uses STREAM_MODE from info.py as the default value
#         return await self.get_bot_setting(bot_id, 'GLOBAL_WATCH_MODE', STREAM_MODE)

#     async def update_watch_mode_status(self, bot_id, enable):
#         await self.update_bot_setting(bot_id, 'GLOBAL_WATCH_MODE', enable)

# # Add these two new functions inside the Database class in users_chats_db.py

#     async def watch_for_all_status(self, bot_id):
#         # Default is False (premium-only)
#         return await self.get_bot_setting(bot_id, 'WATCH_FOR_ALL', False)

#     async def update_watch_for_all_status(self, bot_id, enable):
#         await self.update_bot_setting(bot_id, 'WATCH_FOR_ALL', enable)
...
# Add these two new functions inside the Database class in users_chats_db.py
    ...
    async def get_stream_mode(self, bot_id):
        # Modes: "all", "premium", "off". Default is "premium".
        return await self.get_bot_setting(bot_id, 'STREAM_MODE_STATUS', "premium")

    async def update_stream_mode(self, bot_id, mode):
        await self.update_bot_setting(bot_id, 'STREAM_MODE_STATUS', mode)
...
# Add these two new functions inside the Database class in users_chats_db.py
    ...
    async def sendall_mode_status(self, bot_id):
        return await self.get_bot_setting(bot_id, 'SEND_ALL_PREMIUM', SEND_ALL_MODE_PREMIUM_ONLY)

    async def update_sendall_mode_status(self, bot_id, enable):
        await self.update_bot_setting(bot_id, 'SEND_ALL_PREMIUM', enable)




# Add these three functions inside the Database class in users_chats_db.py

    async def get_daily_limit(self, user_id: int):
        """Gets the user's file count for the current day."""
        today = datetime.datetime.now(pytz.timezone('Asia/Kolkata')).date().isoformat()
        user_data = await self.daily.find_one({"user_id": user_id, "date": today})
        if user_data:
            return user_data.get("count", 0)
        return 0

    async def increment_daily_limit(self, user_id: int):
        """Increments the user's file count for today. Creates the record if it doesn't exist."""
        today = datetime.datetime.now(pytz.timezone('Asia/Kolkata')).date().isoformat()
    ...
    async def reset_daily_limits(self):
        """Optional: Clears the daily limit collection. Can be used for maintenance."""
        await self.daily.delete_many({})
        return "All daily limits have been reset."
